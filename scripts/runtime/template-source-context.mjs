...
export async function resolveTemplateSourceContext(
  ...
  const report = {
    ...
    direct: {
      item: Boolean(templateDocument?.item),
      actor: Boolean(templateDocument?.actor),
      object: Boolean(templateDocument?.object)
    },
    attempted: [],
    matched: [],
    notes: []
  ...
  if (directItem) {
    ...
    emitResolutionLog("resolved", report, snapshot, {
      item: directItem,
      actor,
      caster
    }, emitDebug);
  ...
  const actorFromIds =
    actor ??
    resolveActorFromIds(collectCandidatesFromPaths(templateDocument, ACTOR_ID_PATHS, "actor-id"), report);
  actor = actorFromIds ?? actor ?? null;
  ...
  if (!item && actor) {
    item = findActorItemFromActivityId(
      actor,
      collectActivityIdCandidates(templateDocument),
      report
    );
  }
  ...
  if (!caster) {
    caster = pickFirstDefined(
      resolveActorFromIds(
        collectCasterIdCandidates(templateDocument),
        report
      ),
      actor
    );
  ...
  if (item) {
    recordMatch(report, "resolved-item", "final.item", item.uuid);
  } else {
    report.notes.push("No linked Item could be resolved from the template metadata.");
  }

  if (actor) {
    recordMatch(report, "resolved-actor", "final.actor", actor.uuid);
  }

  if (caster) {
    recordMatch(report, "resolved-caster", "final.caster", caster.uuid);
  }

  emitResolutionLog(item ? "resolved" : "unresolved", report, snapshot, {
    item,
    actor,
    caster
  }, emitDebug);
...
export function collectTemplateSourceDebugSnapshot(templateDocument) {
  ...
  return {
    ...
    width: templateDocument?.width ?? null,
...
async function resolveFromReferenceCandidates(candidates, report) {
  for (const candidate of dedupeCandidates(candidates)) {
    ...
    recordAttempt(
      ...
      candidate.path,
      value,
      item || actor ? "resolved-related-document" : "resolved-unrelated-document"
    );
...
function resolveActorFromIds(candidates, report) {
  for (const candidate of dedupeCandidates(candidates)) {
    ...
    recordAttempt(
      ...
      candidate.path,
      candidate.value,
      actor ? "resolved-actor-id" : "actor-id-not-found"
    );
...
async function resolveItemFromActorAndIds(actor, candidates, report) {
  ...
  for (const candidate of dedupeCandidates(candidates)) {
    ...
    recordAttempt(
      ...
      candidate.path,
      candidate.value,
      item ? "resolved-item-id" : "item-id-not-found-on-actor"
    );
...
function findActorItemFromActivityId(actor, candidates, report) {
  if (!actor) {
    return null;
  }
  ...
  for (const candidate of dedupeCandidates(candidates)) {
    for (const item of actor.items ?? []) {
      if (!itemHasActivityId(item, candidate.value)) {
        continue;
      }
      ...
      recordAttempt(
        ...
        candidate.path,
        candidate.value,
        "resolved-activity-id-via-actor-item"
      );
      recordMatch(report, candidate.kind, candidate.path, item.uuid);
      return item;
    }
    ...
    recordAttempt(
      ...
      candidate.path,
      candidate.value,
      "activity-id-not-found-on-actor"
    );
  }
...
function collectActivityIdCandidates(templateDocument) {
  return [
    ...collectCandidatesFromPaths(templateDocument, [
      ["flags", "dnd5e", "activityId"],
      ["flags", "dnd5e", "activity", "id"],
      ["flags", "dnd5e", "usage", "activityId"],
      ["flags", "dnd5e", "origin", "activityId"],
      ["system", "activityId"]
    ], "activity-id")
  ];
}

function collectCasterIdCandidates(templateDocument) {
  return collectCandidatesFromPaths(templateDocument, [
    ["flags", MODULE_ID, "casterId"],
    ["flags", "dnd5e", "casterId"],
    ["flags", "dnd5e", "origin", "casterId"],
    ["system", "casterId"]
...
function walkCandidateStrings(currentValue, currentPath, collector) {
  ...
  if (Array.isArray(currentValue)) {
    currentValue.forEach((entry, index) => {
      walkCandidateStrings(entry, `${currentPath}[${index}]`, collector);
    });
    return;
  }
...
function itemHasActivityId(item, activityId) {
  const activities = item?.system?.activities;
  if (!activities || !activityId) {
    return false;
  }

  if (typeof activities.get === "function" && activities.get(activityId)) {
    return true;
  }

  if (Array.isArray(activities)) {
    return activities.some((activity) => activity?._id === activityId || activity?.id === activityId);
  }
  ...
  if (isPlainObject(activities)) {
    ...
    return Object.values(activities).some(
      (activity) => activity?._id === activityId || activity?.id === activityId
    );
  }
...
function normalizeResolvedItemFromUnknown(resolvedDocument) {
  ...
  for (let depth = 0; depth < 6 && current; depth += 1) {
    ...
    const relatedItem = normalizeResolvedItem(current?.item ?? null);
    if (relatedItem) {
      return relatedItem;
    }
...
function normalizeResolvedActorFromUnknown(resolvedDocument) {
  ...
  for (let depth = 0; depth < 6 && current; depth += 1) {
    ...
    const relatedActor = normalizeResolvedActor(current?.actor ?? null);
    if (relatedActor) {
      return relatedActor;
    }
...
function emitResolutionLog(status, report, snapshot, context, emitDebug) {
  ...
  debug(
    `Template source context ${status}.`,
    {
